  ...
  return (
    <div style={{ maxWidth: "500px", margin: "auto", padding: "20px" }}>
      ...
      <form onSubmit={submitScout}>
        ...
        <h2>Auton: {auton}</h2>
        <input type="range" min="1" max="10" step="1" value={auton}
        ...
        <div style={{ display: "flex", gap: "10px", marginBottom: "30px" }}>
          {["none","L1","L2","L3"].map(c=>(
            <button type="button" key={c} style={buttonStyle(climb===c)} onClick={()=>setClimb(c)}>
              {c}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: "10px", marginBottom: "30px" }}>
          {["1","2","3"].map(m=>(
            <button type="button" key={m} style={buttonStyle(movement===m)} onClick={()=>setMovement(m)}>
              {m}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: "10px", marginBottom: "30px" }}>
          {["none","1","2","3"].map(i=>(
            <button type="button" key={i} style={buttonStyle(intake===i)} onClick={()=>setIntake(i)}>
              {i}
            </button>
          ))}
        </div>
        ...
        <input type="range" min="1" max="10" step="1" value={accuracy}
